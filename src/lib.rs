...
async fn initialize_wgpu(window: &Window) -> (Handle, State) {
...
impl Handle {
    fn reconfigure(&self, state: &State) {
...
impl State {
    fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) -> &Self {
...
pub async fn run() {
    ...
    let window = match WindowBuilder::new().build(&event_loop) {
        Ok(window) => window,
        Err(error) => panic!("Unable to open window: {:?}", error),
    };
        ...
        match event {
                ...
                if !handle.input(event) {
                    match event {
                        ...
                        WindowEvent::Resized(physical_size) => {
                            handle.reconfigure(state.resize(*physical_size));
                        }
                        WindowEvent::ScaleFactorChanged { new_inner_size, .. } => {
                            handle.reconfigure(state.resize(**new_inner_size));
                        }
                        _ => {}
                    }
                }
            ...
            Event::RedrawRequested(window_id) if window_id == window.id() => {
                ...
                match handle.render() {
                    ...
                    Err(wgpu::SurfaceError::Lost) => handle.reconfigure(&state),
                    // The system is out of memory, we should probably quit
                    Err(wgpu::SurfaceError::OutOfMemory) => *control_flow = ControlFlow::Exit,
                    // The underlying surface has changed, update swap chain
                    // Happens when window is resized too small
                    Err(wgpu::SurfaceError::Outdated) => eprintln!("Outdated"),
